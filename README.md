...
- Integración con servicio RAG para búsqueda por emoción (`VITE_RAG_BASE_URL`).
...
# (Opcional) Para FER real, levanta `moodtune_fer`. Para búsquedas por emoción, levanta `moodtune_rag`.
# En desarrollo puedes:
#  - Usar el proxy '/fer-proxy' sin configurar nada (apunta a http://localhost:8081 por defecto)
#  - Usar el proxy '/rag-proxy' para RAG (apunta a http://localhost:8010 por defecto)
#  - O definir VITE_FER_BASE_URL para cambiar el destino del proxy (FER)
#  - O definir VITE_FER_ENDPOINT_URL con el endpoint completo (ej: http://localhost:8081/infer)
#  - O definir VITE_RAG_BASE_URL para apuntar al servicio RAG real
npm install
npm run dev
```
...
```bash
cd moodtune_frontend
# Opcional: exportar VITE_API_BASE_URL, VITE_FER_ENDPOINT_URL y VITE_RAG_BASE_URL para el build
# PowerShell: $env:VITE_API_BASE_URL="http://localhost:8000"; $env:VITE_FER_ENDPOINT_URL="http://localhost:8081/infer"; $env:VITE_RAG_BASE_URL="http://localhost:8010"
# Bash:      export VITE_API_BASE_URL=http://localhost:8000; export VITE_FER_ENDPOINT_URL=http://localhost:8081/infer; export VITE_RAG_BASE_URL=http://localhost:8010

docker compose up --build
# App en http://localhost:8080
```

Nota: las variables `VITE_` de Vite se resuelven en tiempo de compilación. Si cambias `VITE_API_BASE_URL` o `VITE_RAG_BASE_URL`, recompila la imagen.

## Página "Explorar" (RAG)
- Ruta protegida: `/explore`.
- Permite seleccionar una emoción (Feliz, Triste, Intenso, Relajado) y buscar canciones usando `POST /rag/search`.
- Muestra título/artista, métricas (valence/energy), preview de audio (cuando disponible) y enlace a Spotify cuando hay `uri`.
- Configura el backend RAG con `VITE_RAG_BASE_URL` o usa el proxy `/rag-proxy` en desarrollo.
